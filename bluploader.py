...
def create_torrent(path,basename,arguments,torrentpath):
   if arguments["--torrentdir"]=="temp":
       torrent= "dottorrent -p -t https://blutopia.xyz/announce"+ path +"  "+ torrentpath.name
       output=torrentpath.name
   else:
       torrent= "dottorrent -p -t https://blutopia.xyz/announce "+ path +" "+arguments["--torrentdir"]
...
def IMDBtoTMDB(imdbid,format,arguments):
  ...
  url="https://api.themoviedb.org/3/find/tt" + str(imdbid) +"?api_key="  #! /usr/bin/env python3
...
Usage:
    ...
    Options:
     ...
     --font <font> font for mtn thumbnail
     --compress <compress_png> compress images requires oxipng yes or no
     --announce <blutopia_upload_announce> [default:https://blutopia.xyz/announce]
     --announce
...
def createconfig(arguments):
    ...
    if arguments['--font']==None:
        arguments['--font']=config['general']['font']
    if arguments['--announce']=="https://blutopia.xyz/announce":
...
+arguments['--tmdb']+"e&language=en-US&external_source=imdb_id"
  list=requests.get(url)
  if(format=="TV"):
       format='tv_results'
  if(format=="Movie"):
